...
Class Map<K,V>
	...
	Notes:
		...
		- Fast initialization from flexible data
	@example
		Local m:=New Map<String,Int>(New Variant[]("one",1,"two",2))
		Print m 'outputs: {[one][1]}
	#end
	Method New( k:Variant[] ) 	Where 
									(K=Object Or K=String Or K=Int Or K=Float Or K=Short Or K=Long Or K=Double Or K=Bool) And
									(V=Object Or V=String Or V=Int Or V=Float Or V=Short Or V=Long Or V=Double Or V=Bool)
		
		Assert((k.Length Mod 2) = 0,"Pair missing: array length must be even.")
		Local len:=k.Length/2

		For Local i:=0 Until len+2 Step 2
			Add(Cast<K>(k[i]),Cast<V>(k[i+1]))
		Next
	End
	...
	Note: K as Object allows indexing
	#end
	Method Reset( keys:K[], values:V ) 	Where 
										K=Object Or 
										K=String Or
										K=Int Or K=Float Or 
										K=Short Or K=Long Or K=Double Or K=Bool Or
										K=Variant
		Clear()
		Local this:=New Map<K,V>(keys,values)
		Self._root=this._root
	End

	#rem monkeydoc Resets a new prefilled map with keys and values created from pointed arrays.
	@author iDkP from GaragePixel
	@since 2025-07-02
	The values will fill the entire map.
	...
	#end
	Method Reset( keys:K[], values:V[] ) 	Where 
											(K=Object Or K=String Or K=Int Or K=Float Or K=Short Or K=Long Or K=Double Or K=Bool Or K=Variant) And
											(V=Object Or V=String Or V=Int Or V=Float Or V=Short Or V=Long Or V=Double Or V=Bool Or V=Variant)
		Clear()
		Local this:=New Map<K,V>(keys,values)
		Self._root=this._root
	End
	...
	#rem monkeydoc Resets a new indexed map with values from an array.
	@author iDkP from GaragePixel
	@since 2025-07-02
	The values will fill the entire map.
	The keys will be natural sequence integers 32 bits.
	...
	#rem monkeydoc Search if a contained value exists as the value of a key.
	@author iDkP from GaragePixel
	@since 2025-07-18
	@param key The key.
