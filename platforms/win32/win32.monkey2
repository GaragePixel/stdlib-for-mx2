...
This is just enough to create a window and get a simply message loop running. 

Sibly (2018) : More to come once I get c2mx2 running again...

iDkP from GaragePixel (2025): More has come, since you did not have the desire 
or the time to take care of it before leaving us a relative part of your genius.
Added some interesting variables to handle native win32 drawing,
transparent window, hierarchy, layout and kind of window. Please take a look^^
...
Namespace stdlib.platforms.win32
...
Const WS_POPUP:DWORD ' Added by iDkP from GaragePixel
' Added by iDkP from GaragePixel (https://docs.microsoft.com/en-us/windows/win32/winmsg/extended-window-styles)
Const WS_EX_APPWINDOW:DWORD ' Added by iDkP from GaragePixel
Const WS_EX_COMPOSITED:DWORD ' Added by iDkP from GaragePixel
Const WS_EX_TRANSPARENT:DWORD ' Added by iDkP from GaragePixel
Const WS_EX_CONTROLPARENT:DWORD ' Added by iDkP from GaragePixel
Const WS_EX_LAYERED:DWORD ' Added by iDkP from GaragePixel
Const WS_EX_MDICHILD:DWORD ' Added by iDkP from GaragePixel (https://en.wikipedia.org/wiki/Multiple-document_interface)
Const WS_EX_NOACTIVATE:DWORD ' Added by iDkP from GaragePixel
Const WS_EX_TOOLWINDOW:DWORD ' Added by iDkP from GaragePixel
Const WS_EX_TOPMOST:DWORD ' Added by iDkP from GaragePixel
...
Function SetForegroundWindow:BOOL_(	hWnd:HWND )	
	
Const WM_PAINT:DWORD	
Const WM_DESTROY:DWORD	

Const CS_HREDRAW:DWORD
Const CS_VREDRAW:DWORD
Const CS_OWNDC:DWORD
