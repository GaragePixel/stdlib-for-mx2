...
#rem from https://github.com/richgel999/miniz

	Miniz is a lossless, high performance data compression library 
	in a single source file that implements the zlib (RFC 1950) and 
	Deflate (RFC 1951) compressed data format specification standards.

 	It supports the most commonly used functions exported by the zlib library, 
 	but is a completely independent implementation 
	...
	and reading/writing/appending .ZIP format archives. 	
 	...
 	it also has a specialized real-time compressor function.
...
Struct mz_zip_archive_file_stat
	...
	Field m_filename:stdlib.plugins.libc.char_t Ptr
	Field m_comment:stdlib.plugins.libc.char_t Ptr
End
...
Function mz_zip_reader_extract_to_heap:Void Ptr( pZip:mz_zip_archive Ptr,file_index:Int,pSize:stdlib.plugins.libc.size_t Ptr,flags:Int )
Function mz_zip_reader_extract_file_to_heap:Void Ptr( pZip:mz_zip_archive Ptr,pFilename:CString,pSize:stdlib.plugins.libc.size_t Ptr,flags:Int )
