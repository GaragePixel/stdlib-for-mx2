...
#end
Class DataBuffer Extends stdlib.system.resource.Resource
	...
	#end
	Method New( length:Int,byteOrder:ByteOrder=stdlib.system.memory.ByteOrder.LittleEndian )
	...
	Method New( data:Void Ptr,length:Int,byteOrder:ByteOrder=stdlib.system.memory.ByteOrder.LittleEndian )
	...
	Method Resize( length:Int )
		...
		stdlib.plugins.libc.memcpy( data,_data,Min( length,_length ) )
		GCFree( _data )
		_length=length
		_data=data
	End
	...
	Method CopyTo( dst:DataBuffer,srcOffset:Int,dstOffset:Int,count:Int )
		...
		stdlib.plugins.libc.memmove( dst._data+dstOffset,_data+srcOffset,count )
	End
	...
	A compression level of 1 gives best speed, 9 gives best compression.
	...
	Method Compress:DataBuffer( compressionLevel:Int=-1 )
		...
		Local destLen:=stdlib.plugins.zlib.compressBound( Length )
		Local dest:=New DataBuffer( destLen+8 )
		dest.PokeUInt( 0,Length )
		dest.PokeUInt( 4,~Length )
		Local destLenOut:=destLen
		Local r:=stdlib.plugins.zlib.compress2( dest.Data+8,Cast<stdlib.plugins.zlib.z_uLong Ptr>( Varptr destLenOut ),Data,Length,compressionLevel )
		If r<>stdlib.plugins.zlib.Z_OK RuntimeError( "Error compressing data: zlib return code="+r )
	...
	Method Decompress:DataBuffer()
		...
		Local r:=stdlib.plugins.zlib.uncompress( dest.Data,Cast<stdlib.plugins.zlib.z_uLong Ptr>( Varptr destLen ),Data+8,Length-8 )
		If r<>stdlib.plugins.zlib.Z_OK
			dest.Discard()
			Return Null
		Endif
		Return dest
