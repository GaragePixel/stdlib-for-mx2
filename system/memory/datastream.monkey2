...
#end
Class DataStream Extends stdlib.system.io.Stream
	...
	#rem monkeydoc For extend usage purpose...
	#end
	Method New() Virtual 
	End
	...
	Method Read:Int( buf:Void Ptr,count:Int ) Override
		...
		stdlib.plugins.libc.memcpy( buf,_buf.Data+_off+_pos,count )
	...
	Method Write:Int( buf:Void Ptr,count:Int ) Override
		...
		stdlib.plugins.libc.memcpy( _buf.Data+_off+_pos,buf,count )
