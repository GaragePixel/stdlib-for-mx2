...
Function to_ticks:Long( time:stdlib.plugins.libc.time_t )
	
	Local secs:Long
	
	If stdlib.plugins.libc.sizeof( time )=8
		secs=Cast<Long Ptr>( Varptr time )[0]
	Else If stdlib.plugins.libc.sizeof( time )=4 
...
Function to_time:stdlib.plugins.libc.time_t( ticks:Long )
	
	Local secs:=ticks/TimeSpan.TicksPerSecond
	
	Local time:stdlib.plugins.libc.time_t
	
	If stdlib.plugins.libc.sizeof<stdlib.plugins.libc.time_t>()=8 
		time=Cast<stdlib.plugins.libc.time_t Ptr>( Varptr secs )[0]
	Else If stdlib.plugins.libc.sizeof<stdlib.plugins.libc.time_t>()=4
		Local isecs:=Int( secs )
		time=Cast<stdlib.plugins.libc.time_t Ptr>( Varptr isecs )[0]
	Else
		RuntimeError( "time_t error" )
	Endif
...
On some 32 bit platforms, it may also be suject to the year 2038 problem:
...
Class Time
	...
	#end
	Method New()
		Local ticks:=to_ticks( stdlib.plugins.libc.time( Null ) )
		Init( ticks )
	End
	
	Method New( day:Int,month:Int,year:Int,hours:Int,minutes:Int,seconds:Int )
		
		Local tm:stdlib.plugins.libc.tm_t
		...
		Local ticks:=to_ticks( stdlib.plugins.libc.mktime( Varptr tm ) )
	...
	Field _ticks:Long
	Field _tm:stdlib.plugins.libc.tm_t
	...
	Method Init( ticks:Long )
		...
		Local p:=stdlib.plugins.libc.localtime( Varptr time )
